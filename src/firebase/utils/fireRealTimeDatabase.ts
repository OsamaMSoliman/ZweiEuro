...
export function in_crement(db: Database, coinId: string, letter: string) {
    crement(ref(db, `${baseFolder}/${coinId}/collection/${letter}`), 1);
    if (getAuth().currentUser?.displayName)
        set(ref(db, `${baseFolder}/${coinId}/modifiedBy`), getAuth().currentUser?.displayName);
}
export function de_crement(db: Database, coinId: string, letter: string) {
    crement(ref(db, `${baseFolder}/${coinId}/collection/${letter}`), -1);
    if (getAuth().currentUser?.displayName)
        set(ref(db, `${baseFolder}/${coinId}/modifiedBy`), getAuth().currentUser?.displayName);
}
...
export function uploadCoinData(db: Database, newCoinData: ICoinData) {
